...
/**
 * This class provides tools to calculate the CCITT - CRC 16 codes and verification.
 *
 * It uses a look up table approach ported from linux
 * @see https://github.com/torvalds/linux/blob/master/lib/crc-ccitt.c
 * @see https://github.com/torvalds/linux/blob/master/include/linux/crc-ccitt.h
 */
internal class CrcUtility {
    companion object {
        ...
        /**
         * Get the CRC check sum from a [message] of [length]. The output buffer is initialized to
         * [crcInit]
         * @return the CRC16 check sum from the message.
         */
        fun calculateCcittCrc16(crcInit : Int, message: ByteArray, length: Int): Short {
            var crc16 = crcInit
            for (i in 0 until  length) {
                crc16 = crcCcittByte(crc16, message[i])
            }
        ...
        /**
         * This is an internal function to calculate the CRC code for an single byte hold in [message]
         */
        private fun crcCcittByte(crc :Int, message: Byte): Int{
            val tablePosition = crc xor message.toInt() and 0x00FF
            return (crc shr 8 and 0xFF) xor lookUpTable[tablePosition]
        }
